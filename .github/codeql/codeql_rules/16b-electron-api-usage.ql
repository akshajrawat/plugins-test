...
select node, "Unauthorized Native API Usage: The plugin is importing the raw `electron` module directly. Verify which property is being accessed (e.g., clipboard, dialog). Instruct the developer to use the equivalent official Joplin API (e.g., `joplin.clipboard`) instead." + " " + msg
