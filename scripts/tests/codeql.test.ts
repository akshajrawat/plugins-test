...
describe('CodeQL Security Rules Validation', { timeout: 300000 }, () => {
    ...
    before(() => {
        ...
        execSync(`codeql database create "${DB_PATH}" --language=javascript --source-root="${TESTS_DIR}"`, { stdio: 'ignore' });

        console.log('🔍 [2/2] Analyzing database against 16 custom rules (This may take a moment)...');
        execSync(`codeql database analyze "${DB_PATH}" "${CODEQL_RULES_DIR}" --format=sarif-latest --output="${RESULTS_PATH}" --threads=0`, { stdio: 'ignore' });
